...
const järjestäLukumääränMukaan = (laskuri: { [avain: string]: number }) =>
  Object.entries(laskuri)
    ...
    const laskeMaakuntaEsiintymätAmmatilla = (profiilit: any[], ammatti: string) => {
      const laskuri: { [avain: string]: number } = {};
      ...
      profiilit.forEach((p) => {
        ...
        const ammatit = [
          ...(Array.isArray(p.ammatit) ? p.ammatit : []),
          ...(Array.isArray(p.valitut_ammatit) ? p.valitut_ammatit : [])
        ];
    
        if (!ammatit.includes(ammatti)) return;
...
export default function Kandit() {
  // State for storing data from JSON file
const [ammatitKokonaismaara, setAmmatitKokonaismaara] = useState<{ [key: string]: number }>({});

useEffect(() => {
  fetch("/data/ammatit_kokonaismaara.json")  // Fetch from the public/data folder
    .then((res) => res.json())
    .then((data) => setAmmatitKokonaismaara(data))
    .catch((err) => console.error("Virhe ammatit_kokonaismaara.json latauksessa:", err));
}, []);
  ...
  const [tunnettuMaara, setTunnettuMaara] = useState<number>(0);
...
const kaikkiOsaamisdata = järjestäLukumääränMukaan(laskeEsiintymät(profiilit, "osaamiset"));
  ...
  useEffect(() => {
    fetch("/data/profiilit.json")
      .then((res) => res.json())
      .then((data) => setProfiilit(data.filter((p: any) => p != null)));
  }, []);
  ...
  const hae = () => {
    ...
    setTimeout(() => {
      const tulos = profiilit.filter((p) => {
        ...
        const maakuntaosuma =
          maakuntaValinta === "" ||
          (p.työnhakualue || []).includes(maakuntaValinta);
...
const ammattidata = järjestäLukumääränMukaan(laskeEsiintymät(profiilit, "ammatit"))
  .map(({ nimi, lkm }) => ({
    nimi,
    lkm: Math.round(lkm * skaalauskerroin)
  }))
  .slice(0, 20);
...
const skaalaaAmmatit = () => {
  ...
  const [ammatillinenMaakuntaJakauma, setAmmatillinenMaakuntaJakauma] = useState<any[]>([]);
const skaalatutAmmatit = skaalaaAmmatit();

// TÄHÄN KOHTAAN LISÄTÄÄN:
useEffect(() => {
  if (!valittuAmmattiKartalle || profiilit.length === 0) {
    setAmmatillinenMaakuntaJakauma([]);
    return;
  }

  const raakadata = laskeMaakuntaEsiintymätAmmatilla(profiilit, valittuAmmattiKartalle);
  const yhteensaRaakana = Object.values(raakadata).reduce((sum, lkm) => sum + lkm, 0);
  const tunnettu = ammatitKokonaismaara[valittuAmmattiKartalle] || 0;

  const skaalauskerroin = yhteensaRaakana > 0 ? tunnettu / yhteensaRaakana : 1;

  const jakauma = järjestäLukumääränMukaan(raakadata).map(({ nimi, lkm }) => ({
    nimi,
    lkm: Math.round(lkm * skaalauskerroin)
  }));

  setAmmatillinenMaakuntaJakauma(jakauma);
  setTunnettuMaara(tunnettu);
}, [valittuAmmattiKartalle, profiilit]);
...
{temTilasto && (
  <div className="mt-10 bg-blue-50 border border-blue-200 rounded p-6 max-w-3xl mx-auto">
    ...
    <p className="text-center text-sm text-gray-700 mb-4">
      TEM:n ilmoittama kokonaismäärä: <strong>{temTilasto?.kokonaismaara ? temTilasto.kokonaismaara.toLocaleString() : "Ei tietoa"} hlö</strong>
    </p>
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm text-gray-800">
      {temTilasto?.maakunnat &&
        Object.entries(temTilasto.maakunnat).map(([nimi, lkm]) => (
          <div key={nimi}>
            <strong>{nimi}:</strong> {lkm ? Number(lkm).toLocaleString() : "Ei tietoa"} hlö
          </div>
        ))}
    </div>
    <p className="mt-6 text-xs text-gray-600 text-center leading-snug">
      ...
      julkaistuun työttömien työnhakijoiden kokonaismäärään ({temTilasto?.kokonaismaara ? temTilasto.kokonaismaara.toLocaleString() : "Ei tietoa"} hlö).
...
<div>
  ...
  <ul className="list-disc pl-5 text-sm">
    {ammattidata.map(({ nimi, lkm }) => (
      <li key={nimi}>{nimi} ({lkm ? lkm : "Ei tietoa"} henkilöä)</li>
    ))}
  </ul>
</div>
